...
struct Data_Tree {
    ...
    bool empty=true, is_array=false, is_buffer=false, retry=false,
         is_generic=false, is_smem=false, is_own=false, is_borrow=false;
    ...
    bool IsFromArena() const;
    bool IsInteger() const;
    bool IsBuffered() const;
    bool HasGeneric() const;
    bool IsLayoutUnpack(Data_Tree R) const;
    bool Has(std::string type) const;
    std::vector<Data_Tree> FilterLayoutDims() const;
};




extern std::map<std::string, Data_Tree> fn_ret_dt;
